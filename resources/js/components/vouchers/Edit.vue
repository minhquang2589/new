<template>
    <Dashboard page-title="Vouchers - Edit">
        <form class="max-w-md mx-auto" @submit.prevent="voucherEdit">
...
export default {
    ...
    methods: {
        ...
        async getVoucherData() {
            try {
                ...
                const response = await axios.get(
                    `/api/voucher/edit/${voucherId}`
                );
                if (response.data.success == true) {
                    const voucherData = response.data.vouchersData;
                    this.voucher_code = voucherData.voucher_code;
                    this.start_datetime = voucherData.start_datetime;
                    this.end_datetime = voucherData.end_datetime;
                    this.discount_value = voucherData.discount_value;
                    this.voucher_quantity = voucherData.voucher_quantity;
                    this.status = voucherData.status;
                } else {
        ...
        async voucherEdit() {
            try {
                const voucherId = this.$route.params.id;
                const response = await axios.post("/api/voucher/update", {
